<?php

namespace App\Domain\ApplePay\ApplePayLib\tests\Feature;

use App\Domain\ApplePay\ApplePayLib\Decode\Decoder;
use App\Domain\ApplePay\ApplePayLib\Message\Request\DecodeRequest;
use App\Domain\ApplePay\ApplePayLib\Message\Response\DecodeResponse;
use phpseclib3\File\X509;
use Tests\TestCase;

class DecodeTest extends TestCase
{
    public function testDecodeOk()
    {
        $token = [
               'data' => '',
               'signature' => '',
               'header' => [
                   'publicKeyHash' => 'USXSgL3feiU5sYa3OYlWoyqXHW9fy1VS2Us1Y3qulMw=',
                   'ephemeralPublicKey' => 'MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE4DpxzUf/4B1eJofZ732YNcWBa1g1zF6PMMxirvGxnY5g0/cCA7TrGOj4Nf4QINEcjnjWekmb7rlgQ3nMDGcobg==',
                   'transactionId' => 'a0c9334679695d439f9a2e21a18b469ef9c6ed3a81b4dde914e99a92a1824348'
               ],
               'version' => 'EC_v1'
       ];

        $data = [
            'paymentData' => $token,
            'rootCACertificateContent' => file_get_contents(base_path('/AppleRootCA-G3.cer')),
            'expirationTime' => 315360000,
            'privateKey' => 'MHcCAQEEIAbgRQC9lID5Fh7Hq0HRDiVBleN4exTkkiVNR2yPRBgmoAoGCCqGSM49AwEHoUQDQgAEI7cXhwfAsi/voXyNslttP8ujgXKRctOfZi4WPUKOudny9UcDRl2ePrx5qxL4x4ETt0MoZqs6wj3tlx4ZkYfc6g==',
            'merchantId' => 'merchant.placetopay-test'
        ];

        $response = Decoder::make(DecodeRequest::fromArray($data))->decode();
        $this->assertInstanceOf(DecodeResponse::class, $response);
        $this->assertSame('5454947285823094', $response->applicationPrimaryAccountNumber);
        $this->assertSame('270430', $response->applicationExpirationDate);
        $this->assertSame('840', $response->currencyCode);
        $this->assertSame('10', $response->transactionAmount);
        $this->assertSame('050110030273', $response->deviceManufacturerIdentifier);
        $this->assertSame('3DSecure', $response->paymentDataType);
        $this->assertSame('APVZnyUjXBYyAAT/XtLCAoABFA==', $response->onlinePaymentCryptogram);
        $this->assertNull($response->eciIndicator);
    }
}
